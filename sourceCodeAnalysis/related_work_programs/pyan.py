...
class CallGraphVisitor(object):
    ...
    def visitGetattr(self, node):
        ...
        if isinstance(self.last_value, Node) and self.last_value.orig_node in self.scopes and node.attrname in \
                self.scopes[self.last_value.orig_node].defs:
            verbose_output('getattr %s from %s returns %s' % (
            node.attrname, self.last_value, self.scopes[self.last_value.orig_node].defs[node.attrname]))
...
def main():
    ...
    if options.dot:
        print
        v.to_dot(draw_defines=options.draw_defines,
    ...
    if options.tgf:
        print
        v.to_tgf(draw_defines=options.draw_defines,
                 draw_uses=options.draw_uses)
